Dokumentasi Komprehensif (Frontend & Backend): Halaman Input Penjualan Harian (Stok) - PADAN
1. Spesifikasi Frontend UI/UX (Antarmuka Pengguna)

Halaman Input Penjualan Harian pada menu Stok dirancang untuk memandu rutinitas penutupan warung (closing) secara cepat dan tanpa gesekan kognitif.
A. Komponen & Struktur Layout Layar

    Top Bar & Header:

        Menampilkan logo aplikasi PADAN dan label halaman "Stok" di bagian paling atas, lengkap dengan ikon notifikasi (bell) dan avatar profil akun.

        Badge penanda status: "RUTINITAS TUTUP WARUNG" (berwarna hijau matcha lembut) disandingkan dengan tanggal aktif (Hari Ini, 24 Okt 2024).

        Judul utama: Input Penjualan Harian dengan sub-teks instruksional: "Catat porsi terjual sebelum tutup kasir untuk menjaga akurasi prediksi stok esok hari."

    Widget Toggle Toko Libur / Tutup:

        Kartu khusus dengan ikon kalender/toko, menyertakan label chip "Opsional".

        Teks penjelasan: "Aktifkan jika warung libur agar AI tidak mencatat angka 0 sebagai penurunan tren peminat."

        Dilengkapi tombol Switch interaktif di sebelah kanan.

    Daftar Menu & Porsi Terjual (Daftar Menu & Porsi Terjual):

        Header bagian menampilkan total menu aktif (contoh: 4 Menu Aktif).

        Setiap item menu direpresentasikan dalam kartu putih bersih dengan border tipis (1px solid #E2DDD5) dan sudut melengkung (20px):

            Foto & Nama Menu: Thumbnail menu di sebelah kiri, diikuti nama menu utama (misal: Nasi Goreng Spesial) serta status target vs sisa porsi (Target: 50 porsi • Sisa: 2 porsi).

            Badge Akurasi/Persentase: Badge persentase hijau di pojok kanan atas kartu (misal: 96%, 100%).

            Quick Preset Chips: Tombol cepat di bawah nama menu untuk pengisian instan (misal tombol Habis (50) dan pilihan angka riwayat seperti 45).

            Numeric Quantity Stepper: Kontrol interaktif di sebelah kanan berupa tombol minus (–), angka jumlah porsi aktif dengan tabular lining numbers, dan tombol plus (+).

    Widget Ringkasan AI Harian (Ringkasan AI Harian):

        Kartu analitik dengan badge "Sangat Efisien".

        Menampilkan angka statistik utama dengan ukuran besar: 145 porsi terjual dari 150 target (96.7% efisiensi).

        Progress bar linier berwarna hijau tua penuh di bawah angka statistik.

        Kotak catatan wawasan AI di bagian bawah: "Menu Terlaris: Nasi Goreng (+12% vs minggu lalu). Bahan sisa minimal! Hanya 5 porsi total belum terserap hari ini...".

    Tombol Aksi Bawah (Action CTAs):

        Primary CTA: Tombol pill penuh berwarna hijau sage (#3A5A40) bertuliskan ikon dokumen dan teks "Simpan Data Penjualan Hari Ini".

        Secondary Action: Tombol tonal sekunder berlatar muda bertuliskan "Simpan sebagai Draf".

    Bottom Navigation Bar:

        Docked di bagian bawah layar dengan 5 item: Beranda, Stok (dalam kondisi aktif terbungkus pill hijau #E8F0EA), Limbah, Harga, dan Akun.

2. Spesifikasi Backend & Logika Sistem (Backend Architecture)

Sistem backend menggunakan kerangka kerja RESTful API berbasis FastAPI dengan basis data relasional MariaDB.
A. Struktur Skema Basis Data (MariaDB)

    Tabel daily_sales (Pencatatan Penjualan Harian):

        id (INT, Primary Key, Auto Increment)

        user_id (INT, Foreign Key ke tabel users)

        date (DATE, tanggal pencatatan transaksi)

        is_holiday_toggle (BOOLEAN, bernilai true jika fitur "Toko Libur" diaktifkan)

        created_at (TIMESTAMP)

    Tabel daily_sales_items (Detail Porsi Per Menu):

        id (INT, Primary Key, Auto Increment)

        daily_sales_id (INT, Foreign Key ke daily_sales.id)

        menu_id (INT, Foreign Key ke tabel menus)

        target_portions (INT, target porsi awal dari rekomendasi AI)

        sold_portions (INT, jumlah porsi akhir yang diinput/disesuaikan kasir)

        remaining_portions (INT, sisa porsi fisik)

B. Desain Endpoint API (FastAPI)

    Endpoint Simpan Data Penjualan (POST /api/v1/sales/daily-record)
...

        {
          ...
          "items": [
            ...
            { "menu_id": 3, "sold_portions": 25 },
            { "menu_id": 4, "sold_portions": 40 }
          ]
        }

        Logika Backend:

            Memeriksa apakah is_holiday_toggle bernilai true. Jika ya, sistem backend mencatat status libur pada tanggal tersebut agar algoritma Prophet/XGBoost mengabaikan data kosong dan tidak membaca angka 0 sebagai penurunan tren pasar.

            Menyimpan data rekapitulasi ke dalam tabel daily_sales dan rincian per menu ke daily_sales_items.

            Menghitung ulang persentase efisiensi harian dan memperbarui metrik kerugian yang berhasil dicegah secara otomatis.

            Mengembalikan respons kode 201 Created beserta ringkasan analitik efisiensi untuk ditampilkan pada kartu ringkasan UI.

    Endpoint Simpan Draf (POST /api/v1/sales/save-draft)

        Fungsionalitas: Menyimpan sementara data porsi yang sedang diinput kasir tanpa menguncinya ke model prediksi utama, mencegah kehilangan data jika aplikasi tertutup secara tidak sengaja sebelum finalisasi tutup warung.
